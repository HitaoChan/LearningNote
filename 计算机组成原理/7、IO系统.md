...
把原来的一个CPU工作周期分成两个时间片T1和T2分别给CPU和DMA,使CPU和DMA交 替访存，适用于CPU工作周期大于主存存取周期的情况。这种方法不存在总线的申请和归还，总线使用权通过T4aT2分时控制，有较高的DMA传输效率。
...
3. I/O设备要求数据传输的同时CPU也要求访存，此时I/O优先于CPU访存，I/O需要挪用一两个存取周期,b也就是CPU在执行访问主存的指令过程中插入了DMA请求，并挪用一两个存取周期使得CPU延缓了一两个存储周期才访存。
